...
import { FileText, MoreVertical, Trash2, Edit2, Copy, Search, LayoutGrid, FilePlus2, Loader2, Download, AlertCircle } from 'lucide-react';
...
export default function FilesPage() {
  ...
  const handleDuplicate = async (id: string) => {
    try {
      ...
      toast.success('Nhân bản thành công', { id: 'dup' });
      fetchDocuments();
    } catch (err: any) {
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 text-zinc-400">
            ...
            <p className="font-medium text-sm">Đang tải tài liệu...</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-zinc-400 border-2 border-dashed border-zinc-200 rounded-2xl bg-white">
            ...
            <p className="font-semibold text-zinc-600 mb-1">Không tìm thấy tài liệu nào</p>
            <p className="text-sm">Hãy thử tìm kiếm khác hoặc tạo mới một tài liệu.</p>
          </div>
        ) : (
          <>
            <div className="mb-4 text-xs text-zinc-500 italic">
              * Lưu ý: Các tài liệu ở trạng thái Nháp (DRAFT) sẽ tự động bị xoá sau 24 giờ. Vui lòng bấm vào để "Lưu" và hoàn tất.
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredDocs.map((doc) => (
              <div key={doc.id} className="group flex flex-col bg-white border border-zinc-200 rounded-xl overflow-hidden hover:border-primary/50 hover:shadow-md transition-all">
                
                {/* Preview Mockup Area */}
                <div className="h-32 bg-zinc-50 border-b border-zinc-100 p-4 relative overflow-hidden flex items-start justify-center">
                ...
                <div className="p-4 flex flex-col gap-2 relative">
                  {editingId === doc.id ? (
                    <form onSubmit={handleRename} className="flex gap-2">
                      ...
                      />
                      <button type="submit" className="text-xs bg-primary text-white px-2 rounded font-semibold hover:opacity-90">Lưu</button>
                      <button type="button" onClick={() => setEditingId(null)} className="text-xs bg-zinc-100 text-zinc-600 px-2 rounded font-semibold hover:bg-zinc-200">Huỷ</button>
                    </form>
                  ) : (
                    <div className="flex items-center gap-2 pr-6">
                      <h3 className="font-semibold text-zinc-900 text-sm truncate" title={doc.name}>{doc.name}</h3>
                      {doc.status === 'draft' && (
                  ...
                  <p className="text-xs text-zinc-500">
                    Cập nhật: {new Date(doc.updated_at).toLocaleString('vi-VN', { dateStyle: 'short', timeStyle: 'short' })}
                  </p>
                  ...
                  {!editingId && (
                    <div className="absolute top-4 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                       <button onClick={() => { setEditingId(doc.id); setEditName(doc.name); }} className="p-1.5 text-zinc-400 hover:text-primary hover:bg-primary/5 rounded" title="Đổi tên">
                         <Edit2 className="w-3.5 h-3.5" />
                       </button>
                       <button onClick={() => handleDuplicate(doc.id)} className="p-1.5 text-zinc-400 hover:text-primary hover:bg-primary/5 rounded" title="Nhân bản">
                         <Copy className="w-3.5 h-3.5" />
                       </button>
                       <button onClick={() => setDeletingId(doc.id)} className="p-1.5 text-zinc-400 hover:text-red-500 hover:bg-red-50 rounded" title="Xoá">
      ...
      {deletingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-zinc-600 mb-6">Bạn có chắc chắn muốn xoá tài liệu này không? Hành động này không thể hoàn tác.</p>
            <div className="flex justify-end gap-3">
              <button 
                onClick={() => setDeletingId(null)}
                className="px-4 py-2 font-semibold text-sm text-zinc-600 bg-zinc-100 hover:bg-zinc-200 rounded-lg transition-colors"
              >
                Huỷ
              </button>
              <button 
                onClick={handleDelete}
                className="px-4 py-2 font-semibold text-sm text-white bg-red-600 hover:bg-red-700 rounded-lg shadow-sm transition-colors"
              >
                Xoá Tài Liệu
              </button>
            </div>
...
// Need to import FolderKanban since I used it above
